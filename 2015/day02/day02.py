...
# Puzzle input was saved to a text file.  Read this in, but each new line
# (measurement) is a new list element.  Remember to trim off the trailing
# newline break at the end.
with open("day02_input.txt", "r") as myfile:
	src_data = myfile.read().rstrip('\n')
	data_list = src_data.split('\n')

# Need prod() function similar to sum() function in order to caluclate product
# of list
from operator import mul
def prod(iterable):
	return reduce(mul, iterable, 1)

# For each list element, split the expression of measurements into individual
...
for index in range(len(data_list)):
	...
	# calculate the surface area + slack for each gift individually, and
	# load into a new list
	calc_area = (2 * (meas[0]*meas[1] + meas[1]*meas[2] + meas[2]*meas[0]))
	+ (prod(meas) / max(meas)) data_calc.append(calc_area)
...
for index in range(len(data_list)):
        ...
        for package in data_list[index]:
                meas = [int(n) for n in data_list[index].split('x')]
	# Calculate ribbon length needed for each gift individually, and load
	# into a new list
