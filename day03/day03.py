...
# Start at origin (0,0) and read the next direction from dirs[].
# Take the previous point's coordinates as a tuple, and increment just the appropriate part.
# Save the new coordinate as the next point, then add it to the list of visited homes.
...
# Count duplicates
# Tuples can be hashed and used as keys in a dictionary {}, so do that and keep track of which homes we've visited. A dictionary is used because it stores key-value pairs, and we want to count how many times we visit each house, identified by its coordinates as a key.
visited = {}
for house in coords:
	if house in visited:
		visited[house] += 1
...
# Part Two
# Keep track of coordinates with two different lists
# Santa takes coordinates with odd indices, Robo-Santa takes coordinates with even
...
for index in range(len(dirs)):
	# If even, Robo-Santa will follow the directions, so pull its last coordinates. Otherwise, Santa.
