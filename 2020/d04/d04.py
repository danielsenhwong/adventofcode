...
with open('d04/d04_input.txt') as f:
    d04_input = f.read()
    f.close()
print(d04_input)
passports = d04_input.split("\n\n")
print(passports)

complete = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid', 'cid']
...
for p in passports:
    ...
    if len(r) == 7:
        result = [i for i in r if i.startswith('cid:')]
        if not result:
            check = True
            valid+=1
    elif len(r) == 8:
        ...
        for line in r:
            ...
            if len(field) == 2:
                ...
                if label == 'byr':
                    ...
                    if re.match(r'^[#]{1}[0-9a-f]{6}$', value):
                        print(field, value)
                        p_valid+=1
                elif label == 'ecl':
